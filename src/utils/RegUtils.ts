...
module RegUtils {
	...
	*/
	...
	export function checkArrInGroups(arr: Array<any>, size: number): Array<any> {
	...
	export function setActionData(obj: Object): void {
		if (obj.hasOwnProperty("json")) {
			let proObj: Object = obj["json"];
			if (proObj.hasOwnProperty("actMsg")) {
				let proArr: Array<Object> = proObj["actMsg"];
				if (!GlobalData.gameNeedData) {
					GlobalData.gameNeedData = new ActionVO();
				}
				for (let i: number = 0; i < proArr.length; i++) {
					let obj: any = proArr[i];
					GlobalData.gameNeedData.id = i;
					GlobalData.gameNeedData.actId = parseInt(obj.actId + "");
					GlobalData.gameNeedData.actName = obj.actName;
					GlobalData.gameNeedData.videoName = obj.video;
					GlobalData.gameNeedData.videoPoster = obj.videoCover;
				}
			}
		}
	}

	export function setGameParam(obj: Object): void {
		if (obj.hasOwnProperty("param")) {
			let gameData: any = obj["param"];
			if (!GlobalData.gameNeedData) {
				GlobalData.gameNeedData = new ActionVO();
			}
			if (gameData.deviceId) {
				GlobalData.gameNeedData.devideID = gameData.deviceId + "";
				GlobalData.gameNeedData.courseID = gameData.showId + "";
			}
		}
	...
	export function setLogoData(obj: Object): void {
		GlobalData.logoArr = [];
		if (obj.hasOwnProperty("logoData")) {
			let str: any = obj["logoData"];
			GlobalData.logoArr = str.split("data");
			for (let i: number = 0; i < GlobalData.logoArr.length; i++) {
				GlobalData.logoArr[i] = "data" + GlobalData.logoArr[i];
			}
			GlobalData.logoArr.shift();
		} else {
			let data: any = RES.getRes("template_json");
