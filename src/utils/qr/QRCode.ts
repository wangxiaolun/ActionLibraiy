...
module qr {
    export class QRCode{
         ...
         * color 颜色
         * */
        public static create(msg:string,color=0, width:number=200,height:number=200,errorCorrectLevel=2,taget:egret.Sprite=null):egret.Sprite{
          var _htOption = {
                width : width,
                height : height,
        ...
        public static draw(m:qr.QRCodeModel ,_htOption):egret.Sprite{
            ...
            for (var row = 0; row < nCount; row++) {
                for (var col = 0; col < nCount; col++) {
                    ...
                    if(b){
                        //sc.graphics.clear();
                        sc.graphics.beginFill(_htOption.color,1);
